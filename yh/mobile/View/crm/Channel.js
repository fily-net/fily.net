...
$View.crm.Channel = function (j) {
    ...
    function layout() {
        owner = args.p.adElm('', 'div').cn('wp hp tac lh30 c_6').h('需求未确定, 待定!');
        return;
        var comArgs = {
    ...
    function onTabClick(obj) { mainList.loadAjax('m=SYS_CM_CRM&action=pagingForRightsPM&jsonCondition={"proType":' + obj.name + '}&ifCount=1&pageIndex=1&pageSize=20&dataType=json&keyFields=id, merchantName as title, dbo.SYS_TRANS_USER(cPerson) as cPerson, convert(varchar(20),cTime, 120) as cTime'); }
