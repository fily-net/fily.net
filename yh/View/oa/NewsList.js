...
$View.oa.NewsList = function (j) {
    ...
    var _html = '<div class="title"><a _tk="SYS_CM_NEWS" _tid="{0}" class="icon-compact-line-r"></a><div class="_title">{1}</div><div class="_info"><span style="margin-right:15px;">发布人：{2}</span><span>发布于：{3}</span></div></div></div>';
    function setDefault(j) { args = $.Util.initArgs(j, args); }
    function layout() {
        owner = args.p.adElm('', 'div').cn('wp hp');
        var comArgs = {
            'root': { head_h: 30, foot_h: 25, cn: 'b0', title: '养护信息列表', icon: 'icon-glyph-th-list', toolBarSkin: 'Button-default', toolBarAry: [{ text: '回到首页', name: 'back', icon: 'icon-glyph-arrow-left', css: 'margin-top:3px;', cn: 'mr10'}], onToolBarClick: onToolBarClick },
