$.NameSpace('$View.pm');
$View.pm.PMInfo2 = function (j) {
    var me = this, _fn = function () { };
    var owner;
    var args = { p: $DB, title: '', proId: 0, type: null, onNext: _fn };
    var coms = {}, popTips, eBody, preStep, currStep, nextStep, btnConfirm, tools, mainTab, subTab, cicp;
    ...
    function layout() {
        ...
        eBody = coms.mainLayout.eHead; fileList = coms.fileList; tools = coms.rootTips.toolBar;
        tools.evt('onSuccess', function (obj) { tools = obj.ButtonSet; btnConfirm = tools.items['446']; me.loadPro(args.proId); })
    }

    function onToolBarClick(obj) {
        switch (obj.Name) {
            case '448':
                MConfirm.setWidth(250).show('确认终止项目(不能恢复)?').evt('onOk', function () {
                    $.Util.ajax({
                        args: 'm=SYS_CM_PM&action=stopPro&proId=' + args.proId,
                        onSuccess: function () { MTips.show('终止成功', 'ok'); me.mainList.refresh(); me.loadPro(args.proId); }
                    });
                });
                break;
            case '446':
                MConfirm.setWidth(250).show('确定确认此操作?').evt('onOk', function () {
                    $.Util.ajax({
                        ...
                        onSuccess: function () {
                            ...
                            mainTab.getItem(currStep).Button.setEnabled(true).fireClick(); _confirms = ',';
                            args.onNext();
                        }
                    });
                });
            ...
            case '444':
                ...
                break;
            case '450':
                if (cicp) { mainTab.setSelTab(cicp.Name); loadOverData(cicp); } else { MTips.show('请先切换至"创意初评"'); }
                break;
            case 'back':
            ...
            $.Util.ajax({
                ...
                onSuccess: function (d) {
                    ...
                    for (var i = 0; i < _dLen; i++) {
                        var _name = +_dAry[i].name; _dAry[i].idx = i;
                    ...
                    if (+_pInfo.state == 449) { tools.hide(); } else { delayShowAllTools(true); showConfirm(false); tools.show(); }
                }
            });
        }
    }
    ...
    function initCurrStep(obj) {
        ...
        switch (+obj.Args.idx) {
            case 0:
                var _fc_ary = [
                    { name: '项目名称-0', title: '项目名称', group: { name: 'g3', width: 310} },
                    { name: '公司名称-0', title: '公司名称', group: 'g3' },
                    { name: '创作团队-0', title: '创作团队', group: 'g3' },
                    { name: '目标观众性别-0', title: '目标观众性别', group: 'g3', comType: 'Select', gtID: 131 },
                    { name: '目标观众年龄-0', title: '目标观众年龄', group: 'g3' },
                    { name: '动画时长-0', title: '动画时长', group: { name: 'g2', width: 310} },
                    { name: '孵化人-0', title: '孵化人', group: 'g2' },
                    { name: '制作形式-0', title: '制作形式', group: 'g2' },
                    { name: '故事简述-0', title: '故事简述', group: 'g2' },
                    { name: '世界设定-0', title: '世界设定', group: 'g2' },
                    { name: '角色简介-0', title: '角色简介', group: { name: 'g1', width: 310} },
                    { name: '画面风格-0', title: '画面风格', group: 'g1' },
                    { name: '音乐风格-0', title: '音乐风格', group: 'g1' },
                    { name: '联系人-0', title: '联系人', group: 'g1' },
                    { name: '联系方式-0', title: '联系方式', group: 'g1' }
                ];
                        ...
                        subTab = new $.UI.Tab({
                            ...
                            onTabClick: function (subObj) {
                                $.Util.ajax({
                                    args: 'm=SYS_CM_PM&action=getRights&extid=' + subObj.Name,
                                    onSuccess: function (d) {
                                        if (+d.get(0)) { onPersonTwoTabClick(obj, subObj); } else { allNoRights(subObj.Body); }
                                    }
                                });
                            }
                        });
            ...
            case 3:
                var _fc_ary = [
                    { name: '项目投资方案-0', title: '项目投资方案', comType: 'FileUploader', group: { name: 'g3', width: 310} },
                    { name: '项目执行计划-0', title: '项目执行计划', comType: 'FileUploader', group: 'g3' }
                ];
                var _form = new $.UI.Form({ p: _body, ifFixedHeight: false, hidden: { proId: args.proId, step: obj.Name }, items: _fc_ary, onSubmit: function (obj) { return onFormSubmit(obj, function () { showConfirm(true); }) } });
                loadFormData(_form, function (dAry) { if (dAry.length) { showConfirm(true); } else { showConfirm(false); } });
                break;
    ...
    function onPersonOneTabClick(obj, subObj) {
        ...
        var _form = new $.UI.Form({
            p: _body,
            items: [
                { title: '项目评分', name: '项目评分-0', comType: 'Radios', sons: [{ value: 1, text: 1 }, { value: 2, text: 2 }, { value: 3, text: 3 }, { value: 4, text: 4 }, { value: 5, text: 5}], group: 'info' },
                { title: '评估意见', name: '评估意见-0', comType: 'TextArea' }
            ],
            ifFocus: false,
            ifFixedHeight: false,
    ...
    function onPersonTwoTabClick(obj, subObj) {
        var _body = subObj.Body, _btn = subObj.Button;
        if (_body.h()) { return; }
        var _form = new $.UI.Form({
            p: _body,
            items: [
                { title: '故事简述', name: '故事简述-0', group: { name: 'g1', width: 350} },
                { title: '世界设定', name: '世界设定-0', group: 'g1' },
                { title: '角色简介', name: '角色简介-0', group: 'g1' },
                { title: '画面风格', name: '画面风格-0', group: 'g1' },
                { title: '音乐风格', name: '音乐风格-0', group: 'g1' },
                { title: '创作团队', name: '创作团队-0', group: 'g1' },
                { title: '推荐指数', name: '推荐指数-0', comType: 'Radios', sons: [{ value: 1, text: 1 }, { value: 2, text: 2 }, { value: 3, text: 3 }, { value: 4, text: 4 }, { value: 5, text: 5}], group: { name: 'g2', width: 350} },
                { title: '专业建议', name: '专业建议-0', comType: 'TextArea', group: 'g2' }
            ],
            ifFocus: false,
            ifFixedHeight: false,
            hidden: { proId: args.proId, step: obj.Name, extid: subObj.Name, confirmPerson: _confirms },
    ...
    function initOverStep(obj) { obj.Body.h(''); if (obj.Idx == 1) { allNoRights(obj.Body); cicp = obj; return; }; loadOverData(obj); }
    function loadOverData(obj) {
