...
namespace Fily.Base
{
    ...
    public static class Native
    {
        ...
        public static void writeToPage(string str)
        {
            ...
            try
            {
                string callbackFun = getParameter("callback").ToString();
        ...
        #region getParameter: 根据key值获取参数
        public static string getParameter(string name)
        {
            string _value = HttpContext.Current.Request[name];
            if (isNullEmpty(_value)) { 
                _value = HttpContext.Current.Request.QueryString[name]; 
            }
            return _value;
        }
        #endregion
        ...
        {
            string _module = getParameter("c"), _path = String.Empty;
            if (isNullEmpty(_module)) { _module = getParameter("m"); }
            if (isNullEmpty(_module)) { _module = getParameter("controller"); }
            if (isNullEmpty(_module)) { _module = getParameter("module"); }
        ...
        {
            ifWriteToPage = false;
            string _action = getParameter("action");
            if (isNullEmpty(_action)) { _action = getParameter("api"); }
            if (isNullEmpty(_action)) { writeToPage(getErrorMsg("缺少接口名参数(action or api)")); }
            try { MSession.setClientKey(getParameter("clientkey").ToString()); } catch (Exception) { };
            return _action;
        }
        #endregion
        ...
        public static string getAction(string args)
        {
            ...
            try
            {
                _action = getParameter(args).ToString();
                MSession.setClientKey(getParameter("clientkey").ToString());
        ...
        public static Json checkArgs(string[,] args)
        {
            ...
            for (int i = 0; i < args.GetLength(0); i++)
            {
                ...
                try {
                    _realVal = getParameter(_key).ToString();
                }catch(Exception e) {
                    if (isNull(_defVal))
                    {
        ...
        public static Json initArgs(string[,] args)
        {
            ...
            for (int i = 0; i < args.GetLength(0); i++)
            {
                ...
                string _realVal = getParameter(_key);
