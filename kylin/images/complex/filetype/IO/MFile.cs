...
namespace Fily.IO
{
    ...
    public class MFile
    {
        ...
        /// <returns></returns>
        public string readIcon(string path, string prefix)
        {
            ...
            try
            {
                ...
                while ((_temp = strReader.ReadLine()) != null) {
                    ...
                    if (_temp.IndexOf(prefix) == 0) { _icons.Append(_temp.Split('{')[0].Replace(".", "").Split(':')[0] + "^"); }
                }
        ...
        /// <param name="_speed"></param>
        /// <returns></returns>
        public string getImage(string _fileName, string _fullPath, long _speed)
        {
            string _msg = String.Empty;
            try
            {
                string _name = _fullPath + _fileName;
                HttpRequest _Request = HttpContext.Current.Request;
                HttpResponse _Response = HttpContext.Current.Response;
                FileInfo DownloadFile = new FileInfo(_name);
                if (DownloadFile.Exists)
                {

                    //以数据流的形式根据文件地址打开文件  
                    FileStream stream = new FileStream(_name, FileMode.Open);
                    //获取流的长度  
