...
$.UI.FormItem.EventHandler = function (owner, args) {
    ...
    function onUploadSuccess(obj) {
        ...
        var _eFile = args.eFiles.adElm('', 'a').h('<span class="fa fa-download"></span><a title="预览详情" class="fa fa-file-o" style="margin:5px;" target="_black" href="office_reader.aspx?fid=' + _fObj.id + '"></a><span MTips=' + _fName + '>' + _txt + '</span><span class="fa fa-close" MTips="删除文件" fid="' + _fObj.id + '"></span>').attr('target', '_self').attr('href', 'Module/SYS_CM_FILES.aspx?action=downloadFile&id=' + _fObj.id);
        ...
        switch (type) {
            case 'IconSelector':
                _body.fc().evt('click', function () {
                    ...
                    new $.UI.View({ p: popTips.body, url: 'View/fily/IconSelector.js', onItemClick: function (obj) { var _val = obj.Value; self.setData(_val, _val); popTips.remove(); popTips = null; } });
                });
                break;
            case 'FileUploader':
