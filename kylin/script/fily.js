var $=(function(){
    ...
    _ajax = {
        XHR: function () {
            ...
            me.request = function (argv) {
                if (ifRun) { return; }; ifRun = true;
                var _argv = argv || {},
                    _url = _argv.url,
                    _data = _argv.data,
                    _method = (_argv.method || 'POST').toUpperCase(),
                    _async = _argv.async || true,
                    _contentType = MIME[(_argv.contentType || 'form').toLowerCase()],
                    _onSuccess = _argv.onSuccess || function (){},
                    _onError = _argv.onError || function (){},
                    _onTimeout = _argv.onTimeout || function (){};
                ...
                if (_method == 'GET') {
                    XHR.open(_method, _url + "?" + _data, _async);
                    XHR.onreadystatechange = exec(onStateChange, [_onSuccess, _onError, _timeoutFn]);
    ...
    _load = {
        loadjs: function (url, callback, part) {
            ...
            try { eDom.appendChild(script); } catch (e) { _cbfn(e.message) };
        },
    ...
    $.request = function (argv) {
        if (!$.XHRPOOL) {
            $.XHRPOOL = new _ajax.XHRPool();
        }
        $.XHRPOOL.getXHR().request(argv);
    }
    $.get = function (argv) {
        return argv.method = 'GET', $.request(argv);
    }
    $.post = function (argv) {
        return argv.method = 'POST', $.request(argv);
    }
    $.delete = function (argv) {
        return argv.method = 'DELETE', $.request(argv);
    }
    $.put = function (argv) {
        return argv.method = 'PUT', $.request(argv);
    }
