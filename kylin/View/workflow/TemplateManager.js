...
$View.workflow.TemplateManager = function (j) {
    ...
    function layout() {
        ...
        var comArgs = {
            ...
            'structTips': { head_h: 30, foot_h: 30, icon: 'fa fa-align-justify', title: '系统流程模版列表', cn: 'b0' },
    ...
    function onListClick(obj) {
        ...
        if ((+_tg.getAttr('type')) && flowChart) {
            ...
            tToolBar.reLoadItems([{ name: 'deleteItem', text: '删除选择项', cn: 'mr5', icon: 'fa-minus'}]);
        } else {
            var _tbAry = [
                { name: 'deleteItem', text: '删除选择项', cn: 'mr5', icon: 'fa-minus' },
                { name: 'addItem', text: '新建子项', icon: 'fa-plus' }
            ];
    ...
    function onToolBarClick(obj) {
        ...
        switch (obj.Name) {
            ...
            case 'addItem':
                ...
                popTips = new $.UI.PopDialog({ p: $DB, ePop: obj.Owner, ifClose: true, css: 'max-width:170px;padding:5px 10px 5px 10px;' });
                popTips.init({
                    type: 'Form', ifFixedHeight: false, state: 'Insert',
                    btnItems: [{ name: 'FORM-SYS-SUBMIT', text: '新建(Add)', icon: 'fa-plus', skin: 'Button-blue', align: 'right'}],
                    items: [
