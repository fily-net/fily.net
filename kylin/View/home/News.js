$.NameSpace('$View.home');
$View.home.News = function (j) {
    var me = this, _fn = function () { };
    var owner;
    var args = { p: $DB, data: null, allCount: 0 };
    var _html1 = '<li><div class="title"><a _tk="SYS_CM_NEWS" _tid="{0}" class="fa fa-arrow-circle-o-right"></a><div class="_title">{1}</div><div class="_info"><span style="margin-right:15px;">发布人：{2}</span><span>发布于：{3}</span><span style="margin-right:15px;">生效时间：{4}</span><span>截止时间：{5}</span></div></div></li>';
    ...
    function layout() {
        ...
        owner = args.p.h('<div class="info-panel"><div class="panel-head"><div class="info-title">施工公告</div><a class="info-tag" url="View/oa/NewsList.js">' + args.allCount + '</a></div><div class="panel-body scroll-webkit"><ul>' + _htmls + '</ul></div></div>').evt('click', onOwnerClick);
    }
                ...
                if (_eTitle.ns()) {
                    ...
                    $.Util.ajax({
                        ...
                        onSuccess: function (obj) {
                            var _fAry = eval($.htmlDecode(obj.get(1)) || '[]'), _fLen = _fAry.length, _content = eval($.htmlDecode(obj.get(0)) || '[]')[0].content, _sFile = '';
