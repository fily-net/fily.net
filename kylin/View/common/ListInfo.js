...
$View.common.ListInfo = function (j) {
    var me = this, _fn = function () { };
    var owner;
    var args = { p: $DB, loadApi: '', aHeader: [], fiAry: [], start: 320, table: '', pid: null, onAddItem: _fn };
    var coms = {}, infoF, formBtn, mainList, toolBar;
    ...
    function layout() {
        ...
        var comArgs = {
            ...
            'infoForm': { head_h: 30, ifFixedHeight: false, title: '基本信息', icon: 'fa fa-table', items: args.fiAry, loadApi: 'm=SYS_TABLE_BASE&action=getByID&table=' + args.table, updateApi: 'm=SYS_TABLE_BASE&action=updateByID&table=' + args.table, insertApi: 'm=SYS_TABLE_BASE&action=addRow&table=' + args.table }
        }
        var struct = {
            p: owner,
            type: 'Layout',
    ...
    function onAddItem() {
        ...
        formBtn.setIcon('fa fa-plus').setText('添加基本信息');
        args.onAddItem(infoF);
    }
    function onReset() { infoF.hide(); }
    function onMainListClick(obj) {
        var _id = obj.Target.getAttr('rowId');
        infoF.show().loadDataByID(_id, function () { formBtn.setIcon('fa fa-edit').setText('修改'); });
    }
    ...
    me.loadByPid = function (api, pid) {
        args.pid = pid; api += '&pid=' + pid; mainList.setAttr('ifAjax', false).loadAjax({ args: api });
        return me;
    }
