$.NameSpace('$View.oa');
$View.oa.NewsList = function (j) {
    var me = this, _fn = function () { };
    var owner;
    var args = { p: $DB };
    var coms = {}, paging, eList;
    var _html = '<div class="title"><div class="avatar"><img class="avatar" src="api.aspx?m=SYS_CM_USERS&action=getAvatar&uid={4}" /></div><div class="title-content"><a _tk="SYS_CM_NEWS" _tid="{0}" class="fa fa-arrow-circle-o-right"></a><div class="_title">{1}</div><div class="_info"><span class="attr" style="margin-right:15px;">发布人：<i class="attr-value">{2}</i></span><span class="attr">发布于：<i class="attr-value">{3}</i></span><span style="margin-left:15px;" class="attr time">开始：<i class="attr-value">{4}</i>截止：<i class="attr-value">{5}</i></span></div></div></div></div>';
    function setDefault(j) { args = $.Util.initArgs(j, args); }
    function layout() {
        owner = args.p.adElm('', 'div').cn('wp hp');
        var comArgs = {
            'root': { head_h: 30, foot_h: 30, cn: 'b0', title: '施工公告列表', icon: 'fa fa-th-list', toolBarSkin: 'Button-s1', toolBarAry: [{ text: '回到首页', name: 'back', icon: 'fa fa-arrow-circle-o-left', css: 'margin-top:3px;'}], onToolBarClick: onToolBarClick },
            'paging': { onSelect: loadNews, onClick: loadNews }
        }
        var struct = { p: owner, type: 'Tips', name: 'root', foot: { name: 'paging', type: 'Paging' } }
        coms = $.Util.initUI({ args: comArgs, struct: struct });
        paging = coms.paging; eList = coms.root.body.cn('Waterfall').h('<ul class="ListItem"></ul>').fc().evt('click', onOwnerClick); loadNews();
    }
    ...
    function loadNews() {
        ...
        $.Util.ajax({
            args: 'm=SYS_CM_HOME&action=loadNews&dataType=json&pageIndex=' + _pIdx + '&pageSize=' + _pSize,
                    ...
                    $.Util.ajax({
                        ...
                        onSuccess: function (obj) {
                            ...
                            for (var i = 0; i < _fLen; i++) {
                                var _file = _fAry[i]; _sFile += '<a href="Module/SYS_CM_FILES.aspx?action=downloadFile&id=' + _file.id + '" >' + _file.nodeName + '</a>';
                            }
