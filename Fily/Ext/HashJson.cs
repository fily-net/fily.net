...
namespace Meeko.Ext
{
    public class HashJson: Hashtable
    {
        #region 私有变量的定义
        ...
        public HashJson() : base() { }
        #endregion

        #region HashJson: 通过一个已经存在的Hashtable进行初始化
        public HashJson(Hashtable hash)
            : base()
        {
        ...
        public HashJson(string jsonStr): base()
        {
            HashJson json = HashJson.Parse(jsonStr);
        ...
        #region HashJson: 通过一个已经存在的Hashtable进行初始化
        public static HashJson Parse(string jsonStr)
        {
            HashJson _json = new HashJson();
            try
            {
        ...
        /// </summary>
        /// <returns></returns>
        public virtual HashJson CloneBase()
        {
            HashJson hJson = new HashJson();
            lock (this)
            {
        ...
        public virtual HashJson Concat(string [,] args)
        {
            try
            {
        ...
        public virtual HashJson Concat(HashJson json)
        {
            foreach (DictionaryEntry de in json)
            {
                this.Add(de.Key, de.Value);
        ...
        */
        ...
        #region ToJsonStr: 把自身的对象转成json格式的字符串
        public virtual HashJson AryToHashJson(string [,] args)
        {
            HashJson _json = new HashJson();
            try{
                for (int i = 0; i < args.Length; i++ )
