...
$View.website.AboutUs = function (j) {
    ...
    function layout() {
        var _userHAry = [
            ...
            { name: 'title', type: 'none', title: '标题', width: 120, ifFilter: true, filterItems: ['like'] }
            //{ name: 'url', type: 'none', title: '链接', width: 180 },
            //{ name: 'note', type: 'none', title: '描述', width: 600 }
        ];
        var _userInfo = [
            //{ name: 'type', title: '类型', group: { name: 'g1', width: 600 }, width: 430, ifTrans: true, gtID: 763, comType: 'Select' },
            { name: 'title', title: '标题', group: { name: 'g1', width: 800 }, width: 630, comType: 'Input' },
            { name: 'url', title: '标题图片', group: 'g1', width: 630, comType: 'FileView' },
            { name: 'note', title: '描述', comType: 'RichText', width: 637, group: 'g1' }
        ];
        owner = args.p.adElm('', 'div').cn('wp hp');
        var comArgs = {
            'userTips': {
                head_h: 30, title: '关于我们',
            ...
            'userList': {
                ...
                loadApi: 'm=SYS_TABLE_BASE&table=DL_ABOUT&action=pagingForList&orderCondition={"cTime": "asc"}',
            ...
            'userForm': {
                head_h: 0, 
                loadApi: 'm=SYS_TABLE_BASE&table=DL_ABOUT&action=getByID',
                insertApi: 'm=SYS_TABLE_BASE&table=DL_ABOUT&action=addRow',
                updateApi: 'm=SYS_TABLE_BASE&table=DL_ABOUT&action=updateByID',
                items: _userInfo,
        ...
        switch (obj.Name) {
            ...
            case 'delete':
                ...
                MConfirm.setWidth(250).show('确定删除该' + _ids.length + '项记录?').evt('onOk', function () {
                    $.Util.ajax({
                        args: 'm=SYS_TABLE_BASE&table=DL_ABOUT&action=deleteByIDs&ids=' + _ids.join(','),
