...
$View.projectyh.Supplier = function (j) {
    var me = this, _fn = function () { };
    var owner;
    var args = { p: $DB, table: 'TZ_SUPPLIER' };
    var coms = {}, infoF, formBtn, mainList;
    function setDefault(j) { args = $.Util.initArgs(j, args); }
    function layout() {
        owner = args.p.adElm('', 'div').cn('wp hp');
        var _tbAry = [
            {name:'add',text:'新建供应商', cn:'mr10', icon:'fa fa-plus'},
            {name:'del',text:'删除供应商',icon:'fa fa-close'}
        ];
        var comArgs = {
            'rootBase': { head_h: 30, cn:'b0', toolBarSkin:'Button-default', title: '供应商管理', icon:'fa fa-leaf', toolBarAry: _tbAry, onToolBarClick: onToolBarClick },
        ...
        switch(obj.Name){
            case 'add':
                ...
                var _form = $.global.arrowTips.init({
                    ...
                    btnItems: [
                        { name: 'FORM-SYS-SUBMIT', icon: 'fa fa-plus', text: '新建供应商', skin: 'Button-blue', css: 'margin-left:100px;' }
                    ],
    ...
    function onMainListClick(obj){
        ...
        infoF.reLoadItems(_infoAry, { onSuccess: function (){ infoF.loadDataByID(_id, function (){formBtn.setIcon('fa fa-edit').setText('编辑供应商');}); } });
    }
