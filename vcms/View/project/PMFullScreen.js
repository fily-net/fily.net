...
$View.project.PMFullScreen = function (j) {
    ...
    var _taskAry = [
        ...
        { title: '施工编号', name: 'proCode', type: 'none', width: 240, ifFilter: true, filterItems: ['like'] },
        ...
        { title: '工程类型', name: 'proType', type: 'select', width: 80, ifFilter: true, gtID: 456, filterItems: ['equal'] },
        ...
        { title: '工程区域', name: 'proArea', type: 'select', width: 80, ifFilter: true, gtID: 469, filterItems: ['equal'] },
    ...
    var _taskAry = [
        ...
        { title: '<font color="#C84823">当前状态</font>', name: 'state', type: 'select', width: 120, ifFilter: true, gtID: 505, filterItems: ['equal'] },
        { title: '收单日', name: 'collectTime', type: 'date', width: 145, ifFilter: true, filterItems: ['equal'] },
        { title: '工程编号', name: 'proCode', type: 'none', width: 180, ifFilter: true, filterItems: ['like'] },
        { title: '工程地址', name: 'address', type: 'none', width: 280, ifFilter: true, filterItems: ['like'] },
        ...
        { title: '备注', name: 'note', key: 'note', type: 'none', width: 160, ifEnabledTips: true, ifFilter: true, filterItems: ['like'] }
    ];
    function setDefault(j) { args = $.Util.initArgs(j, args); }
    function layout() {
        owner = args.p.adElm('', 'div').cn('wp hp');
        var _rightApi = 'm=SYS_TABLE_BASE&table=PRO_MG&action=pagingForRightsWFList';
        var _baseApi = 'm=SYS_TABLE_BASE&table=PRO_MG&action=pagingForList';
        if (+args.typeId) {
            _baseApi += '&jsonCondition={"proType": "'+args.typeId+'"}';
        }
        var _toolBarAry = [
            { name: 'switch', text: '切换回标准模式', css: 'margin-top:2px;margin-right:10px;', icon: 'fa-arrow-circle-o-left', skin: 'Button-blue' }
        ];
        var comArgs = {
            ...
            'taskList': { aHeader: _taskAry, loadApi: _baseApi, ifEnabledFilter: true, onSuccess: function (obj) { if (obj.Length) { obj.List.fireClick(0); } }, onTDDoubleClick: onTaskClick, colControls: { header: {}, paging: { pageSize: 15, pageIndex: 1}} }
        }
        var struct = {
            p: owner,
            type: 'Tips',
    ...
    function onToolBarClick(obj) {
        if (obj.name == 'switch') {
            new $.UI.View({ p: args.p, url: 'View/project/PM.js', typeId: args.typeId });
            return false;
        }
        switch (+obj.Name) {
            case 113:
    ...
    function onTaskClick(obj) {
        new $.UI.View({ p: args.p, url: 'View/project/PMInfo.js', proId: obj.eTr.attr('rowid'), typeId: args.typeId });
    }
