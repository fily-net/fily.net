...
$View.workflow.WorkFlowInfo = function (j) {
    ...
    function addDetailItem(p, obj) {
        ...
        if (+obj.sons) {
            var _eTitle = p.adElm('', 'li').cn('node-type-' + obj.type).css('border-left:5px solid #BF0000;').h('<div class="head"><a class="fa fa-angle-right"></a>' + _title + '</div><div class="content"></div>').fc(),
                ...
                $.Util.ajax({
                    ...
                    onSuccess: function (d) {
                        ...
                        switch (_ls) {
                            ...
                            case 2:
                                _fiAry = [
                                    { name: 'roles', title: '部门', comType: 'MultiSelect', loadApi: 'm=SYS_CM_USERS&action=getAllDepts', req: false, sErr: '部门是必填字段' },
