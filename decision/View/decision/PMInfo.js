...
$View.decision.PMInfo = function (j) {
    var me = this, _fn = function () { };
    var owner;
    var args = { p: $DB, title: '', proId: 0, type: null, onNext: _fn };
    var coms = {}, popTips, eBody, preStep, currStep, nextStep, btnConfirm, tools, mainTab, subTab;
        ...
        switch (obj.Name) {
            case '446':
                MConfirm.setWidth(250).show('确定确认此操作?').evt('onOk', function () {
                    $.Util.ajax({
                        args: 'm=SYS_CM_PM&action=nextStep&proId=' + args.proId + '&currStep=' + nextStep + '&nextStep=' + nextStep + '&schedule=' + (_currIdx / _currLen * 100),
                        onSuccess: function () {
                            MTips.show('确认成功', 'ok');
