...
$View.decision.UserManager = function (j) {
    ...
    function layout() {
        var _userHAry = [
            ...
            { name: 'cName', type: 'none', title: '姓名', width: 100, ifFilter: true, filterItems: ['like'] },
            { name: 'mobilePhoneNum', type: 'none', title: '电话', width: 120 },
            { name: 'email', type: 'none', title: '邮箱', width: 150 },
            { name: 'address', type: 'none', title: '通讯地址', width: 150 },
            { name: 'note', type: 'none', title: '备注', width: 150 }
        ];
        var _userInfo = [
            { name: 'avatar', title: '', comType: 'Label', group: { name: 'g1', width: 320 }, type: 'image', ifSubmit: false },
            { name: 'cName', title: '姓名', comType: 'Input', group: 'g1', req: true, sErr:'用户名必填' },
            { name: 'mobilePhoneNum', title: '电话', group: 'g1', comType: 'Input' },
            { name: 'address', title: '通讯地址', group: { name: 'g2', width: 320}, comType: 'Input' },
            { name: 'email', title: '邮箱', group: 'g2', comType: 'Input' },
            { name: 'note', title: '备注', comType: 'TextArea', group: 'g2' }
        ];
        owner = args.p.adElm('', 'div').cn('wp hp');
        var comArgs = {
            'userTips': { head_h: 30, title: '外部用户管理', icon: 'fa fa-user', cn: 'b0', onToolBarMouseDown: onToolBarMouseDown, onToolBarClick: onToolBarClick, toolBarSkin: 'mr5 Button-default', gbsID: 19 },
            'userList': { aHeader: _userHAry, ifEnabledFilter: true, loadApi: 'm=SYS_TABLE_BASE&table=decision_user&action=pagingForList&delFlag=0&jsonCondition={"id,>":2}&orderCondition={"cTime": "asc"}', colControls: { header: {}, paging: {} }, onTDClick: onUserClick, onSuccess: function (obj) { if (obj.Length) { obj.List.fireClick(0); } else { toolBar.fireClick(0); } } },
            'userForm': { head_h: 0, icon_h: 18, loadApi: 'm=SYS_TABLE_BASE&action=getByID&table=decision_user', insertApi: 'm=SYS_TABLE_BASE&action=addRow&table=decision_user', updateApi: 'm=SYS_TABLE_BASE&action=updateByID&table=decision_user', title: '员工档案信息', items: _userInfo, onSubmitSuccess: onFormSubmitSuccess },
    ...
    function onToolBarClick(obj) {
        switch (obj.Name) {
            case '20':
                ...
                infoF.getButton('FORM-SYS-SUBMIT').setIcon('fa fa-plus').setText('添加用户信息');
                break;
            case '21':
    ...
    function delayShowInfo(id) {
        ...
        infoF.loadDataByID(id, function () { infoF.getButton('FORM-SYS-SUBMIT').setIcon('fa fa-edit').setText('修改用户信息'); });
    }
