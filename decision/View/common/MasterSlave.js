...
$View.common.MasterSlave = function (j) {
        ...
        for (var c = 0, _cLen = childs.length; c < _cLen; c++) {
            ...
            switch (_child[0]) {
                case 'Form':
                    ...
                    _com.show().getButton('FORM-SYS-SUBMIT').setIcon('fa fa-edit').setText('修改基本信息');
                    break;
                case 'List':
                    _com.loadAjax({
    ...
    function onAddItem() {
        if (args.onAddFormBefore(infoF) != false) {
            ...
            infoF.getButton('FORM-SYS-SUBMIT').setIcon('fa fa-plus').setText('添加基本信息');
        }
    }
    ...
    function onSlaveMenuClick(iObj, _obj, list) {
        ...
        switch (iObj.Value) {
            ...
            case 'sys_list_add':
                ...
                var _form = new $.UI.Form({ p: popTips.body, ifFixedHeight: false, btnItems: [{ name: 'FORM-SYS-SUBMIT', text: '新建', icon: 'fa fa-plus', css: 'margin-left:102px;'}], insertApi: 'm=SYS_TABLE_BASE&api=addRow&table=' + _obj.table, items: _obj.formItemAry, foot_h: 32, extSubmitVal: { oid: currID} });
