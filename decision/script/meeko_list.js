...
$.UI.List = function (j) {
    ...
    */
    ...
    function getSql4MYSQL(aHeader) {
        ...
        var _hAry = aHeader || args.aHeader, _name = '', sQ = '{0}<tr ';
        var sAttr = '', aCol = [], sCb = '', _firstNode, _name, _sName, _fVis, _ifSI = args.ifShowIcon;
        if (args.ifBindID) { sQ += 'rowId="{0}{1}cast(id as char(20)){1}{0}" '; }
        for (var i = 0, _hLen = _hAry.length; i < _hLen; i++) {
            var col = _hAry[i], _type = col.type || 'none', _sCol = '';
            _name = col.name; _sName = _name;
            if (_type.toLowerCase() == 'select') { col.ifTrans = true; }
            if (col.ifTrans) { _sName = (col.trans || 'SYS_TRANS_GT') + '(' + _name + ')'; }
            if (_name == attr.expandCol) { _sCol += '<div sons="{0}{1}cast(ifnull(' + args.sonsKey + ',{0}{0}) as char(10)){1}{0}" state="0" class="td-node {0}{1}dbo.SYS_SET_ICON_FOR_TREELIST(' + args.sonsKey + ',{0}td-fold{0}){1}{0}" ></div>'; }
            switch (_type.toLowerCase()) {
                case 'checkbox':
                    sCb += '<td><div val="0" class="td-checkbox"><div class="td-checkbox-icon dn"></div></div>';
                    break;
                case 'icon':
                    aCol.push('concat({0}<div class="td-detail" ><div class="image ' + (col.image || '') + '" ></div><div class="td-value" ></div></div>{0}) as ' + _name);
                    break;
                case 'image':
                    aCol.push('concat({0}<div class="td-detail" ><img src="' + (col.path || '') + '{0}{1}' + _name + '{1}{0}" /><div class="td-value" >{0}{1}' + _name + '{1}{0}</div></div>{0}) as ' + _name);
                    break;
                case 'date':
                    aCol.push('concat({0}<div class="td-detail" ><div class="td-text" >{0}{1}SYS_FORMAT_TIME(' + _name + '){1}{0}</div><div class="td-value" >{0}{1}SYS_FORMAT_TIME(' + _name + '){1}{0}</div></div>{0}) as ' + _name);
                    break;
                case 'normal':
                    return '&resultSplit=$';
                case 'operate':
                    ...
                    for (var j = 0, _jLen = _items.length; j < _jLen; j++) {
                        ...
                        switch (_item.type.toLowerCase()) {
                            case 'normal':
                                _sI = '<A rowId="{0}{1}cast(' + (_item.key || 'id') + ' as char(20)){1}{0}" name="' + _item.name + '" href="' + _item.href + '" >';
                                if (_item.icon) { _sI += '<span class="A-icon ' + _item.icon + '" title="' + _item.title + '"></span>'; }
                                if (_item.text) { _sI += '<span class="A-text">' + _item.text + '</span>'; }
                                _sI += '</A>';
                                break;
                    ...
                    aCol.push('concat({0}<div class="td-detail" >' + _sAry.join('') + '</div>{0}) as ' + _name);
                    break;
                case 'process':
                    _name = '{0}{1}cast(ifnull(' + _name + ',0) as char(20)){1}{0}';
                    aCol.push('concat({0}<div class="td-detail" ><div class="td-process"><div class="td-process-value" style="width:' + _name + '%;">' + _name + '%</div></div><div class="td-value">' + _name + '</div></div>{0}) as ' + _name);
                    break;
                case 'attr':
                    var _key = col.key || _name;
                    if (col.isLocalAttr) {
                        ...
                        sAttr += _key + '="{0}{1}cast(replace(ifnull(' + _name + ',{0}{0}),{0}"{0},{0}%24%24{0}) as char(100)){1}{0}" ';
                    }
                    break;
                default:
                    if (!_fVis && _ifSI) {
                        if (typeof _ifSI == 'string') {
                            _sCol += '<div class="td-icon icon-type-{0}{1}cast(' + _ifSI + ' as char(10)){1}{0}"></div>';
                        } else {
                            _sCol += '<div class="td-icon {0}{1}cast(icon as char(50)){1}{0}"></div>';
                        }
                        _fVis = _name;
                    }
                    var _sMT = '';
                    if (col.ifEnabledTips) { _sMT = ' MTips="1" '; }
                    _sName = col.sqlName || _sName;
                    _sCol += '<div class="td-text" ' + _sMT + '>{0}{1}cast(' + _sName + ' as char(500)){1}{0}</div><div class="td-value" >{0}{1}cast(' + _name + ' as char(500)){1}{0}</div>';
                    aCol.push('concat({0}<div class="td-detail" >' + _sCol + '</div>{0}) as ' + _name);
                    break;
            }
        }
        if (args.hasCheckBox) {
            ...
            if (_fNode) {
                aCol.shift();
                _firstNode = '><td>{0}{1}cast(ifnull(' + _fNode + ', {0}{0}) as char(500)),';
            } else {
                alert('必须要有可见列!'); return '';
            }
        }
        if (!aCol.length) { _firstNode = _firstNode.substr(0, _firstNode.length - 1); }
        sQ = 'concat(' + sQ + sAttr + _firstNode + ') as id';
        aCol.unshift(sQ);
        return aCol.join(',').format(_spChar, hyphen);
    }
    ...
    me.getKeyFieldsByHeader = function (aHeader, dbT) {
        var _dbType = dbT || args.dbType;
        if (args.dbType == 'mysql') {
            return getSql4MYSQL(aHeader);
        } else {
            return getSql4MSSQL(aHeader)
        }
        /*
        if (args.dataType == 'html') {

        } else {
            return getQueryFields(aHeader);
        }*/
    }
