...
$View.docflow.MyFlowTask = function (j) {
    ...
    function onTaskDoubleClick(obj) {
        var _urlString = obj.getAttr('url'),
            _taskName = obj.getAttr('nodeName'),
            _urlAry = _urlString.split('?');

        if (_taskName.indexOf('办公用品') != -1) {
        ...
        switch (_url) {
            case 'View/project/PMInfo.js':
                ...
                new $.UI.View({ p: args.p, url: _url, proId: _id, backUrl: 'View/docflow/MyFlowTask.js' });
                break;
