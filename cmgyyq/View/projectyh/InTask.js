...
$View.projectyh.InTask = function (j) {
    var me = this, _fn = function () { };
    var owner;
    var args = { p: $DB, proId: 1 };
    ...
    var _msAry = [
        ...
        { title: '<font color="red">进价(￥)</font>', name: 'price', type: 'none', width: 100, comType: 'KeyInput', ifEdit: true },
        { title: '<font color="red">数量</font>', name: 'number', type: 'none', width: 100, comType: 'KeyInput', ifEdit: true },
        { title: '总值(￥)', name: 'totalSum', union: '$price*$number', type: 'none', width: 100 }
    ];
    var _msDetailAry = [
        { title: '扫描码', name: 'scanCode', type: 'none', width: 120 },
        ...
        { title: '<font color="red">收量</font>', name: 'isnull(detail.number, 0)', type: 'none', width: 100 },
    ...
    function layout() {
        ...
        var comArgs = {
            'root': { head_h: 30, title: '工程收料管理', icon: 'fa fa-bank', cn: 'b0', toolBarSkin: 'mr10 Button-default', gbsID: 105, onToolBarClick: onToolBarClick },
    ...
    function onToolBarClick(obj) {
        switch (+obj.Name) {
            case 108:
                if (!taskInfo) { return; }
                taskInfo.setInstanceId(-1); taskInfo.setTitle('<font color="red">新建施工收料任务</font>'); msInfos = {};
                ...
                var _form = new $.UI.Form({
                    p: _tempL.eHead,
                    submitApi: 'm=SYS_CM_WH_PROJECT&action=create&type=receive&wfIdx=78',
                    items: _fiAry,
                    extSubmitVal: {
                        proId: args.proId
                    },
                    foot_h: 35,
                    btnItems: [
                        { name: 'FORM-SYS-SUBMIT', text: '新建单据', icon: 'fa fa-plus', css: 'margin-left:102px;' },
                        { name: 'selectms', text: '选择物资', icon: 'fa fa-cart-plus', css: 'margin-left:8px;' }
                    ],
                    onClick: onFormClick
                });
                ...
                _form.evt('onSubmit', function (obj) {
                    ...
                    for (var key in msInfos) {
                        ...
                        _count += (+_ms.number) * (+_ms.price);
                        _ms.sum = (+_ms.price) * (+_ms.number);
                        _sAry.push($.JSON.encode(_ms));
                    }
                    if (_costVal != _count.toFixed(2)) {
                        MTips.show('选择物料金额(<font class="fw">' + _count.toFixed(2) + '</font>)和实际单据金额(<font class="fw">' + _costVal + '</font>)不一致，请核对后再提交', 'warn');
                        return false;
                    }
    ...
    function onMSDoubleClick(obj) {
        ...
        $.Util.ajax({
            args: 'm=SYS_TABLE_TREE&action=getNodeByID&table=SYS_WH_MS&dataType=json&id=' + _id + '&keyFields=id,nodeName,scanCode,code,guige,danwei,price,planPrice,avgPrice,lowPrice,highPrice',
    ...
    function onWFLoad() {
        ...
        new $.UI.List({ p: _eMS, ifBindID: false, ifFixedHeight: false, ifEnabledFilter: false, aHeader: _msDetailAry, loadApi: 'm=SYS_CM_WH&action=getMSDetailForTK&tkId=' + tkId + '&type=TK_WH_RECEIVE', colControls: { header: {}} });
    }
