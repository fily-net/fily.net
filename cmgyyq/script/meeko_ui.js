...
$.UI.FormItem = function (j) {
    ...
    function layout() {
        ...
        owner.cn('FormItem ' + args.cn + ' ' + args.comType).css(args.css).h(_h);
                    ...
                    onSuccess: function (obj) {
                        ...
                        for (var i = 0; i < _aLen; i++) {
                            ...
                            var _eFile = _eFiles.adElm('', 'a')
                                .attr('target', '_blank')
                                .attr('href', _url)
                                .h('<a class="fa fa-download" target="_black" href="Module/SYS_CM_FILES.aspx?action=downloadFile&id=' + _fObj.id + '"></a><a title="预览详情" class="fa fa-file-o" style="margin:5px;" target="_black" href="office_reader.aspx?fid=' + _fObj.id + '"></a><span MTips=' + _fName + '>' + _txt + '</span><span class="fa fa-close" MTips="删除文件" fid="' + _fObj.id + '"></span>');
    ...
    me.delFile = function (_e) {
        MConfirm.setWidth(350).show('确定删除该文件?').evt('onOk', function () {
            args.value = args.value.replaceAll(_e.attr('fid') + ',', ''); _e.pn().r();
            $.Util.ajax({
                args: { m: 'SYS_CM_FILES', action: 'delFileOrDir', id: _e.attr('fid') },
                onSuccess: function (obj) {
                    MTips.show('删除成功', 'ok');
                }
            });
            me.set('isChange', true);
        });
    }
...
$.UI.FormGroup = function (j) {
    ...
    */
    ...
    function layout() {
        var _html = '<fieldset class="{0}"><legend><a class="{1} m3"></a><span class="fs14">{2}</span><a class="fa fa-arrow-circle-o-down" expand="meeko"></a></legend><div></div></fieldset>';
...
$.UI.Form = function (j) {
    ...
    */
    ...
    me.getExt = function (key) { return args.extSubmitVal[key]; }
    ...
    */
    ...
    function bindEvent() {
        $D.evt('keydown', function (e) {
            ...
            if (e.keyCode == 46 && wf_me.selElm) { 
                deletePoint(wf_me.selElm.id); 
                wf_me.selElm = null; 
                wf_me.selPoint = null; 
            }
    ...
    function createPoint(j) {
        ...
        elm.onDragStart = function (x, y, e) {
            ...
            if (isPressDown) { elm.dragable = false; } else { elm.dragable = true; }
        }
