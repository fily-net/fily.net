...
<% 
    ...
    switch (action) {
        case "pagingForMSDetail":
            ...
            {
                string _where = "CHARINDEX('"+_args.getValue("code")+"', msCode)<>0", 
                        _filter = MConvert.toFilterSql(_args.getValue("filterCondition").Trim()),
                        _jsonCondition = MConvert.toWhereSql(_args.getValue("jsonCondition"));

                if (_filter.Length > 0)
                {
                    _where += " and " + _filter;
                }
                if (_jsonCondition.Length > 0)
                {
                    _where += " and " + _jsonCondition;
                }
                responseText = baseApi.paging("SYS_WH_MS_DETAIL", _args.getInt("pageIndex"), _args.getInt("pageSize"), _args.getValue("keyFields"), _where, MConvert.toOrderSql(_args.getValue("orderCondition")), _args.getInt("delFlag"));
            }
            break;
        case "importMSIndex":
            args = new string[,] { 
