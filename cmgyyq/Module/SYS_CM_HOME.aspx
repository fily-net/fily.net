    ...
    switch (action) {
        case "pagingForList":
            ...
            if (hash.getBool(Native.CHECK_RESULT))
            {
                ...
                _where += "(datediff(day,bTime,getdate())>0 and datediff(day,getdate(),eTime)>0) or (bTime is null or eTime is null)";
                responseText = baseApi.paging(hash.getValue("table"), hash.getInt("pageIndex"), hash.getInt("pageSize"), hash.getValue("keyFields"), _where, MConvert.toOrderSql(hash.getValue("orderCondition")), hash.getInt("delFlag"));
        ...
        case "init":
            args = new string[,] {
                { "newsFields", "id, title, convert(varchar(20),cTime, 120) as cTime, convert(varchar(20),bTime, 120) as bTime, convert(varchar(20),eTime, 120) as eTime, dbo.SYS_TRANS_USER(cPerson) as cPerson" },
                { "newsIndex", "1" }, 
                { "newsSize", "10" }, 
            ...
            if (hash.getBool(Native.CHECK_RESULT))
            {
                responseText = baseApi.paging(T_news, hash.getInt("newsIndex"), hash.getInt("newsSize"), hash.getValue("newsFields"), "(datediff(day,bTime,getdate())>0 and datediff(day,getdate(),eTime)>0) or (bTime is null or eTime is null)", "cTime desc", hash.getInt("delFlag"));
        ...
        case "loadNews":
            args = new string[,] { 
                { "keyFields", "id, title, convert(varchar(20),cTime, 120) as cTime, dbo.SYS_TRANS_USER(cPerson) as cPerson, cPerson as uid, convert(varchar(20),bTime, 120) as bTime, convert(varchar(20),eTime, 120) as eTime" }, 
                { "pageIndex", "1" }, 
                { "pageSize", "10" },
                { "jsonCondition", "1=1" }, 
                { "jsonOrder", "{\"cTime\":\"desc\"}" },
