        ...
        case "login":           //登陆
            ...
            if (hash.getBool(Native.CHECK_RESULT))
            {
                if (Native.isEmpty(hash.getValue("icCard")))
                {
                    ...
                    Native.write(responseText + ", SQL: "+MString.getSelectStr(T_USERS, "id", "uid='" + hash.getValue("uid") + "' and pwd='" + hash.getValue("pwd") + "'"));
                    if (Native.isEmpty(responseText)) {
                        responseText = Native.getErrorMsg("用户名或密码错误!");
                    } else { MSession.set(responseText, responseText); }
                }
                else {
                    responseText = baseApi.execQuery(MString.getSelectStr(T_USERS, "id", "icCard='"+ hash.getValue("icCard") +"'"));
                    if (Native.isEmpty(responseText)) { responseText = Native.getErrorMsg("IC卡错误!"); } else { MSession.set(responseText, responseText); }
                }
            }
            break;
        case "getExtDepts":
