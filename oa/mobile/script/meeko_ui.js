...
$(function () {
    ...
    if (!window.ValidateTips) { window.ValidateTips = new $.UI.MiniTips({ ifShow: false, css: 'padding:3px 5px;border:1px solid #FFC97C;color:#E52C01;background-color:#FFFEC9;z-index:2500;' }); }
    if (!window.trace) { window.trace = function (v) { if (isIE) { alert(v) } else { console.log(v); } } }
    ...
    $(window).evt('resize', function () { $.UI.DestroyPopElm('window--resize'); if (ValidateTips) { ValidateTips.hide(); } });
    if (isIE) {
        if (!window.debug) { $.global.setImgPath('../'); }
...
$.UI.PopDialog = function (j) {
    ...
    */
    ...
    function layout() {
        ...
        args.owner = owner;
        if (args.ifClose) { owner.adElm('', 'a').cn('pt-close').attr('title', '关闭').evt('click', function () { me.hide(); $.UI.DestroyPopElm('--PopTips-Close'); }); }
...
$.UI.FormItem = function (j) {
    ...
    me.reset = function (b) {
        ...
        switch (me.type) {
            ...
            case 'FileUploader': args.eFiles.h(''); break;
            case 'MultiSelect':
                ...
                $.Util.ajax({
                    ...
                    onSuccess: function (obj) {
                        ...
                        for (var i = 0; i < _aLen; i++) {
                            ...
                            _eFiles.adElm('', 'a').h('<span class="download"></span><span MTips=' + _fName + '>' + _txt + '</span>').attr('target', '_self').attr('href', 'Module/SYS_CM_FILES.aspx?action=downloadFile&id=' + _fObj.id);
                        }
                    }
                });
                break;
...
$.UI.Form = function (j) {
    ...
    */
    ...
    var args = {
        ...
        btnItems: [{ name: 'FORM-SYS-SUBMIT', text: '提交', skin: 'Button-blue', css: 'margin-left:100px;'}],
    ...
    function layout() {
        ...
        me.setIcon(args.icon).setTitle(args.title);
    ...
    me.submit = function () {
        ...
        var _val, _action = args.submitApi, _aAry = [], _hVal = $.Util.toArgsString(args.hidden), _sArgs = '', _infoText = '添加';
        switch (args.state) {
            case 'Insert':
    ...
    me.setIcon = function (v) { eIcon.cn(v + ' m3 fl'); return me; }
...
$.UI.Tips = function (j) {
    ...
    */
    ...
    function layout() {
        var eHead, eFoot, _pos = getXY(args.comMode), _hHtml = '<a style="float:left;margin:' + (args.head_h - args.icon_h) / 2.5 + 'px 3px;"></a><span style="float:left;line-height:30px;"></span>';
        if (args.ifFixedHeight) {
        ...
        if (args.ifResize) {
            resizeDiv = new $.UI.Resizer({ p: owner });
            resizeDiv.onDrag = function () { }
            resizeDiv.onDragEnd = function (_m, _a, _b) { }
        }
        owner.onselectstart = function () { return false; }
    ...
    function layout() {
        switch (args.type) {
            case 'alert':
                ...
                break;
            case 'tips':
                tips = new $.UI.Tips({ ifFixedHeight: args.ifFixedHeight, ifMask: args.ifMask, comMode: 'x-auto', y: 10, css: 'padding: 8px 15px;font-size: 14px;color: #fff;text-shadow: 0 -1px 0 rgba(0, 0, 0, 0.2);' });
                break;
            case 'confirm':
                ...
                me.button = new $.UI.ButtonSet({ p: tips.foot, itemAlign: 'right', items: [{ text: '确定', name: 'confirm', skin: 'Button-blue', cn: 'mr10' }, { text: '取消', name: 'cancle', skin: 'Button-danger', cn: 'mr10'}], onClick: onBtnClick });
                break;
            case 'arrow':
...
$.UI.AttrPanel = function (j) {
    ...
    */
    var me = this, _fn = function () { };
    var args = { p: $DB, width: null, height: null, titleHeight: 25, skin: 'AttrPanel-default', ifFold: true, ifClose: false, ifExpand: true, ifFixedHeight: false, cn: '', css: '', title: '', icon: '', toolBarAry: [], onToolBarClick: _fn, onExpand: _fn };
    var owner, eIcon, eTitle, eContent;
...
$.UI.Calendar = function (j) {
    ...
    */
    ...
    function fillHead(eHead) {
        ...
        eHead.h('<div><a class="yLeft {2}"></a><a class="yCenter cal-text" year="{0}">{0}年</a><a class="yRight {2}"></a><a class="mLeft {2}"></a><a class="mCenter cal-text" month="{1}">{1}月</a><a class="mRight {2}"></a></div>'.format(_year, _month, _iconCn));
...
$.UI.Tab = function (j) {
    ...
    */
    ...
    var args = { p: $DB, items: [], head_h: 35, toolBarAry: [], skin: 'BaseDiv-tab', cn: '', css: '', ifFixedBodyHeight: true, btnSkin: 'Button-tab', loadMode: 'auto', onClose: _fn, onTabClick: _fn, onToolBarClick: _fn };
...
$.UI.FileBrowser = function (j) {
    ...
    */
    ...
    function loadDirFiles(dir, eStart, cbFn) {
        ...
        loadData(dir, function (obj) {
            ...
            if (_sAry.length > 1) {
                ...
                for (var i = 0; i < _len; i++) {
                    var _file = _oAry[i], _type = _file.type, _dir = _file.currPath || '';
                    if (_ePre) { _eLI = _ePre.aeElm('', 'li'); _ePre = _eLI; } else { _eLI = owner.adElm('', 'li'); }
                    _eLI.cn(_type).h(_spaces + '<a class="flag-close" _dir="' + _dir + '" ></a><a class="icon-close"></a><span>' + _file.name + '</span><a class="update"></a><a class="del"></a>').attr('_depth', _depth).attr('name', _file.name).attr('fullName', _file.fullName || '');
                }
            }
            _fn(_len);
        });
    ...
    */
    ...
    function setDefault(j) {
        ...
        htmlTemp = {
            ...
            chain: '<form target="upload" method="post" enctype="multipart/form-data" ><div class="detail" ></div><div><a class="close" title="删除"></a><a><span>选择文件</span><input type="file" name="Uploader_{0}" /></a></div></form>',
    ...
    function layout() {
        ...
        toolBar = new $.UI.ButtonSet({
            ...
            onClick: onOperClick,
            items: [
                { name: 'add', text: '添加', icon: 'icon-glyph-plus-sign' },
                { name: 'start', text: '开始', state: 'stop', icon: 'icon-glyph-circle-arrow-up' }
            ]
        });
        ...
        if (_spLen) {
            ...
            getSpeciFilesState(_fileAry, function (d) {
                ...
                for (var _idx in _eCheckObj) {
                    var _eImg = _eCheckObj[_idx], _val = _checkObj[_idx];
                    _eImg.dc('checking');
                    if (_val) {
    ...
    function uploadCallback(isTimeout) {
        ...
        if (currObj) {
            ...
            if (status == "success") {
                ...
                args.onSuccess({ FileUploader: me, File: _fObj });
            } else {
        ...
        var _obj = {
            f: function () {
                if (_eDetail.chn(1)) {
                    ...
                    currObj = { eImg: $(_eOper.lastChild), eState: _eState, fileName: _strFN, ifSpecial: true };
                    _eForm.action = getUploadUrl() + '&specialFileName=' + _file;
                    _eForm.submit();
                } else {
