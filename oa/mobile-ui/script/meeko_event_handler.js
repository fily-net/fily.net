...
$.UI.FormItem.EventHandler = function (owner, args) {
    ...
    function doEvent() {
        ...
        if (eMark) { eMark.evt('click', function (e) { var e = $.e.fix(e); e.stop(); self.get('onClickBefore')({ FormItem: self, Args: args }); if (type == 'NumUpDown') { clickNumUpDown(e.t); } else { me.fireClick(); }; return; self.get('onClick')({ FormItem: self, Args: args }); }); }
    }
    ...
    function clickSelect() {
        ...
        _tips.init({
            type: 'Menu', gtID: args.gtID, items: args.items, checkedValue: self.getValue(), loadApi: args.loadApi, textKey: args.textKey, valueKey: args.valueKey, hidden: args.hidden, ifShowIcon: args.ifShowIcon,
            onSuccess: function () { _tips.show(); },
            onClick: function (obj) { self.setData(obj.Value, obj.Text); _tips.hide(); args.onSelect(); }
        });
    }
    ...
    function onUploadSuccess(obj) {
        ...
        self.set('value', _val + _fObj.id + ',').set('text', _val + _fObj.id + ',');
        if (_txt.length > 25) { _txt = _txt.substr(0, 15) + '...'; }
        args.eFiles.adElm('', 'a').h('<span class="download"></span><span MTips=' + _fName + '>' + _txt + '</span>').attr('target', '_self').attr('href', 'Module/SYS_CM_FILES.aspx?action=downloadFile&id=' + _fObj.id);
        args.onSuccess({ FormItem: self, FObj: _fObj });
    }
        ...
        switch (type) {
            case 'IconSelector':
                _body.fc().evt('click', function () {
                    popTips = new $.UI.Tips({ comMode: 'x-auto', width: 800, height: 400, y: 100, head_h: 30, ifMask: true, ifClose: true, title: '图标选择器', icon: 'icon-glyph-picture' });
                        ...
                        case 'MultiSelect-user':
                            popTips = new $.UI.Tips({
                                ...
                                onClose: function (obj) {
                                    var users = obj.Tips.UserSelector.getUsers(), _val = $.Util.uniqueArray(users[0]).join(','), _txt = $.Util.uniqueArray(users[1]).join(',');
            ...
            case 'Timer':
                _body.evt('click', function (e) {
                    ...
                    if (!args.lockDate && _e.tagName == 'SPAN' && _e.attr('value')) {
                        var _tips = getPopTips();
                        _tips.set('ePop', _e).init({ type: 'Calendar', nowD: _e.attr('value') }).evt('onClick', function (obj) { var _val = obj.Value; _e.attr('value', _val); _e.h(_val); _tips.hide(); }).show();
                    }
                    if (_e.tagName == 'INPUT') {
                        switch (_e.attr('pk')) {
                            case 'date':
                                var _tips = getPopTips();
                                _tips.set('ePop', _e).init({ type: 'Calendar', nowD: _e.value }).evt('onClick', function (obj) { var _val = obj.Value; _e.value = _val; _tips.hide(); }).show();
                                break;
                            case 'h':
                                var _iAry = [];
                                for (var i = 0; i < 24; i++) { _iAry.push({ name: i, text: i }); }
                                var _tips = getPopTips(60);
                                _tips.set('ePop', _e).init({ type: 'Menu', items: _iAry, checkedValue: _e.value }).evt('onClick', function (obj) { _e.value = obj.Text; _e.pn().attr('hh', obj.Text); _tips.hide(); }).show();
                                break;
                                break;
                            case 'm':
                                var _iAry = [];
...
$.UI.FormItem.onInputClick = function (eInput, args) {
    ...
    function getPopTips() {
        ...
        var _pTips = $.global.popTips, _whCss = 'height:200px;', _bW = eInput.csn('width');
    ...
    function clickSelect() {
        ...
        _tips.init({
            type: 'Menu', items: _items, loadApi: args.loadApi, textKey: args.textKey, valueKey: args.valueKey, hidden: args.hidden,
            onSuccess: function () { _tips.show(); },
            onClick: function (obj) { setData(obj.Value, obj.Text); _tips.hide(); }
        }).show();
    }
...
$.UI.FormItem.getHtml = function (m) {
    var _type = m.comType, _w = +m.width, _h = +m.height, _sBody = '', _sMark = '';
    if (m.ifMark) { _sMark = '<div class="mark">{1}</div>'; _w -= 20; }
    var _sInput = '<input autocomplete="off" name="' + m.name + '" class="input-normal" type="{0}" style="width:' + _w + 'px;height:' + _h + 'px;" />' + _sMark;
    ...
    switch (_type) {
        ...
        case 'RichText':
        case 'TextArea':
            _sBody = '<textarea style="width:' + _w + 'px;height:' + _h + 'px;" class="input-normal"></textarea>';
            break;
        case 'Pwd':
        case 'RePwd':
    ...
    var _html = '<div class="body" >' + _sBody + '</div>';
...
$.UI.FormItem.initArgs = function (args) {
    ...
    var attrObj = {
        AutoComplete: { ifAutoPopHeight: true, dataSrc: null, aHeader: _aHeader, hasCB: _hasCB, oldHeader: _aHeader, key: '', readonly: false, ifSpecial: true },
        ...
        Timer: { noInput: true, popWidth: 250 },
        Label: { path: _imgPath, ifEdit: false, size: 'fixed', type: 'text', value: '', suffix: 'png', title: null, splitChar: '/', imgWidth: 96, imgHeight: 96, readonly: true, noInput: true },
        ...
        TextArea: { height: 60, width: 155, readonly: false },
        ...
        Select: { ifAutoPopHeight: true, ifShowIcon: false, items: [], loadApi: '', textKey: 'nodeName', ifMark: true, valueKey: 'id', hidden: {}, ifSpecial: true, readonly: true, onSelect: _f },
