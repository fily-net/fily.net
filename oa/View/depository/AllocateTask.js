...
$View.depository.AllocateTask = function (args) {
    ...
    var _msAry = [
        ...
        { title: '<font color="red">申调拨数量</font>', name: 'planNum', type: 'none', width: 100, comType: 'KeyInput', ifEdit: true },
        { title: '<font color="red">实调拨数量</font>', name: 'number', type: 'none', width: 100, comType: 'KeyInput', ifEdit: true },
        { title: '<font class="fw c_1 bc_3 p5">当前库存量</font>', name: 'stockNumber', type: 'none', width: 100 },
        { title: '申调拨总值(￥)', name: 'planSum', union: '$planPrice*$planNum', type: 'none', width: 100 },
        { title: '实调拨总值(￥)', name: 'totalSum', union: '$planPrice*$number', type: 'none', width: 100 }
    ];
    var _msDetailAry = [
        ...
        { title: '<font color="red">批次价</font>', name: 'isnull(detail.price, 0)', type: 'none', width: 80 },
        { title: '<font color="red">实际申调拨数</font>', name: 'isnull(detail.number, 0)', type: 'none', width: 120 },
        { title: '<font color="red">计划申调拨数</font>', name: 'isnull(detail.planNum, 0)', type: 'none', width: 120 }
    ];
    var _taskAry = [
        ...
        { title: '源库', name: 'sourceId', ifTrans: true, trans: 'SYS_TRANS_WH', ifFilter: true, type: 'select', loadApi: 'm=SYS_TABLE_TREE&action=getNodesByPid&table=SYS_WH_INDEX&pid=4', filterItems: ['equal'], width: 120 },
        { title: '目标库', name: 'whId', ifTrans: true, trans: 'SYS_TRANS_WH', ifFilter: true, type: 'select', loadApi: 'm=SYS_TABLE_TREE&action=getNodesByPid&table=SYS_WH_INDEX&pid=4', filterItems: ['equal'], width: 120 },
        { title: '调拨人', name: 'cPerson', ifTrans: true, trans: 'SYS_TRANS_USER', type: 'select', textKey: 'uid', ifFilter: true, loadApi: 'm=SYS_CM_USERS&action=getAllUsers', filterItems: ['equal'], width: 80 },
        { title: '调拨日期', name: 'tiaoBoTime', type: 'date', width: 130 },
        ...
        { title: '调拨说明', name: 'note', key: 'note', type: 'none', width: 120, ifEnabledTips: true }
    ];
    var _fiAry = [
        { name: 'code', title: '调拨单', comType: 'Label' },
        { name: 'sourceId', title: '源库', dataType: 'int', comType: 'Select', loadApi: 'm=SYS_TABLE_TREE&action=getNodesByPid&table=SYS_WH_INDEX&pid=4', req: true, onChange: function (obj) { currWHID = obj.Value; } },
        { name: 'whId', title: '目标库', dataType: 'int', comType: 'Select', loadApi: 'm=SYS_TABLE_TREE&action=getNodesByPid&table=SYS_WH_INDEX&pid=4', req: true, onChange: function (obj) { if (obj.Value == currWHID) { MTips.show('源库不能跟目标库一样, 请重新选择'); obj.FormItem.reset(); }; } },
        { name: 'tiaoBoTime', title: '调拨日期', comType: 'Date', req: true },
        { name: 'note', title: '调拨说明', comType: 'TextArea' }
    ];
    function _default() {
    ...
    function _layout() {
        ...
        var comArgs = {
            'root': { head_h: 35, title: '仓库调拨管理', icon: 'icon-glyph-gift', cn: 'b0', toolBarSkin: 'mr10 Button-default', gbsID: 111, onToolBarClick: onToolBarClick },
            ...
            'taskList': { aHeader: _taskAry, loadApi: 'm=SYS_TABLE_BASE&table=TK_WH_ALLOCATE&action=pagingForRightsWFList', ifBindID: false, ifEnabledFilter: true, onSuccess: function (obj) { if (obj.Length) { obj.List.fireClick(0); } else { toolBar.fireClick(0); } }, onTDClick: onTaskClick, colControls: { header: {}, paging: { pageSize: 20, pageIndex: 1 } } }
        }
        var struct = {
            p: owner,
            type: 'Tips',
    ...
    function onWFNextSucc(obj) {
        $.Util.ajax({
            args: 'm=SYS_TABLE_BASE&table=TK_WH_ALLOCATE&action=updateRights&users=' + obj.Node.owner + '&id=' + tkId,
    ...
    function onToolBarClick(obj) {
        switch (+obj.Name) {
            case 120:
                if (!taskInfo) { return; };
                taskInfo.setInstanceId(-1); taskInfo.setTitle('<font color="red">新建调拨任务</font>'); msInfos = {};
                var _eExt = taskInfo.addPanel({ title: '调拨单' }).css('min-height:315px;position: relative;border:1px dashed #e0e0e0;');
                var _tempL = new $.UI.Layout({ p: _eExt, dir: 'we', start: 300, isRoot: true, ifDrag: false, barWidth: 1 });
                var _form = new $.UI.Form({ p: _tempL.eHead, submitApi: 'm=SYS_CM_WH&action=create&type=allocate&wfIdx=77', items: _fiAry, foot_h: 35, btnItems: [{ name: 'FORM-SYS-SUBMIT', text: '新建单据', icon: 'icon-glyph-plus-sign', css: 'margin-left:102px;' }, { name: 'selectms', text: '选择物资', icon: 'icon-glyph-shopping-cart', css: 'margin-left:8px;' }], onClick: onFormClick });
                if (_form.items['scanCode']) { var _scanCodeValue = $.Util.code.FullDate(); _form.items['scanCode'].setData(_scanCodeValue, _scanCodeValue); }
                if (_form.items['code']) { var _code = 'TB' + $.Util.code.FullDate(); _form.items['code'].setData(_code, _code); }
    ...
    function onOperateClick(obj, form) {
        ...
        switch (obj.Name) {
            case 'detail':
                if (!currWHID) { MTips.show('请先选中仓库!', 'warn'); return false; }
                $.Util.ajax({
                    args: 'm=SYS_CM_WH&action=getMSBatchDetail&msId=' + _id + '&totalNum=' + msInfos[_id].number + '&whId=' + currWHID,
                    onSuccess: function (data) {
                        var _batchAry = eval(data.get(0)), _bLen = _batchAry.length, _sHtml = '<li class="fwb" style="height:25px;line-height:25px;"><div style="float:left;width:120px;">库</div><div style="float:left;width:120px;">批次号</div><div style="float:left;width:120px;">进格</div><div style="float:left;width:120px;">剩余数</div><div style="float:left;width:120px;">总数</div><div style="float:left;width:120px;">采购人</div><div style="float:left;width:120px;">创建时间</div><li>';
                        for (var i = 0; i < _bLen; i++) {
                            var _bObj = _batchAry[i];
                            _sHtml += '<li style="height:20px;line-height:20px;"><div style="float:left;width:120px;">' + _bObj.wh + '</div><div style="float:left;width:120px;">' + _bObj.batchCode + '</div><div style="float:left;width:120px;">' + _bObj.price + '</div><div style="float:left;width:120px;">' + _bObj.remainNum + '</div><div style="float:left;width:120px;">' + _bObj.totalNum + '</div><div style="float:left;width:120px;">' + _bObj.cPerson + '</div><div style="float:left;width:120px;">' + _bObj.cTime + '</div><li>';
                        }
    ...
    function onMSDoubleClick(obj) {
        ...
        $.Util.ajax({
            args: 'm=SYS_TABLE_TREE&action=getNodeByID&table=SYS_WH_MS&dataType=json&id=' + _id + '&keyFields=id,nodeName,scanCode,code,guige,danwei,price,planPrice,avgPrice,lowPrice,highPrice,(select number from dbo.SYS_WH_STOCK where msId=' + _id + ' and whId=' + currWHID + ') as stockNumber',
            onSuccess: function (data) {
    ...
    function onWFComplete(obj) {
        MConfirm.setWidth(250).show('流程完成将会修改仓库库存信息！').evt('onOk', function () {
            $.Util.ajax({ args: 'm=SYS_CM_WH&action=onAllocateComplete&tkId=' + tkId });
        });
    }
    function onWFLoad() {
        var _eMS = taskInfo.addPanel({ title: '物资详情' }).css('position: relative;border-left:1px solid #e0e0e0;');
        new $.UI.List({ p: _eMS, ifBindID: false, ifFixedHeight: false, sonsKey: 'detail.sons', pidKey: 'detail.pid', style: 'tree:ms.scanCode', ifEnabledFilter: false, aHeader: _msDetailAry, loadApi: 'm=SYS_CM_WH&action=getMSDetailForTK&tkId=' + tkId + '&type=TK_WH_ALLOCATE&jsonCondition={"detail.pid":0}', colControls: { header: {} } });
    }
