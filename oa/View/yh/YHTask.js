$.namespace('$View.yh');
$View.yh.YHTask = function (args) {
    var me = this, coms, owner, _fn = function () { };
    var _args = { p: $DB, proType: 0, proId: 0, tid: 'YH_TASK', status: 0 };
    ...
    var _taskAry = [
        ...
        { title: '养护名称', name: 'proId', type: 'none', width: 100 },
        { title: '养护类型', name: 'proType', ifTrans: true, type: 'none', width: 100 },
        { title: '申领人', name: 'cPerson', ifTrans: true, trans: 'SYS_TRANS_USER', type: 'select', textKey: 'uid', ifFilter: true, loadApi: 'm=SYS_CM_USERS&action=getAllUsers', filterItems: ['equal'], width: 80 },
        { title: '申领时间', name: 'lingLiaoTime', type: 'date', width: 130 },
    ...
    function _layout() {
        ...
        var comArgs = {
            'root': { head_h: 35, title: '养护领料任务', icon: 'icon-glyph-gift', cn: 'b0', toolBarSkin: 'mr10 Button-default', gbsID: 78, onToolBarClick: onToolBarClick },
            ...
            'taskList': { aHeader: _taskAry, loadApi: 'm=SYS_TABLE_BASE&table=' + args.tid + '&action=pagingForRightsWFList&jsonCondition={"proId": ' + args.proId + ', "proType": ' + args.proType + '}', ifBindID: false, ifEnabledFilter: true, onSuccess: function (obj) { if (obj.Length) { obj.List.fireClick(0); } else { toolBar.fireClick(0); } }, onTDClick: onTaskClick, colControls: { header: {}, paging: { pageSize: 20, pageIndex: 1}} }
        }
        if (+args.status == 350) { comArgs.root.gbsID = -1; }
        var struct = {
            p: owner,
            type: 'Tips',
            name: 'root',
    ...
    function onWFNextSucc(obj) {
        $.Util.ajax({ args: 'm=SYS_TABLE_BASE&table=' + args.tid + '&action=updateRights&users=' + obj.Node.owner + '&id=' + tkId, onSuccess: function () { taskList.refresh(); } });
    }
    ...
    function onToolBarClick(obj) {
        switch (+obj.Name) {
            case 79:
                if (!taskInfo) { return; };
                var _title = '养护';
                if (!args.proId) { _title = '无养护'; }
                taskInfo.setInstanceId(-1); taskInfo.setTitle('<font color="red">新建'+_title+'领料任务</font>'); msInfos = {};
                var _eExt = taskInfo.addPanel({ title: '领料表单' }).css('min-height:315px;position: relative;border:1px dashed #e0e0e0;');
                var _tempL = new $.UI.Layout({ p: _eExt, dir: 'we', start: 300, isRoot: true, ifDrag: false, barWidth: 1 });
                var _form = new $.UI.Form({ p: _tempL.eHead, submitApi: 'm=SYS_CM_WH&action=create&type=yhSend&wfIdx=77', extSubmitVal: { proId: args.proId, proType: args.proType }, items: _fiAry, foot_h: 35, btnItems: [{ name: 'FORM-SYS-SUBMIT', text: '新建单据', icon: 'icon-glyph-plus-sign', css: 'margin-left:102px;' }, { name: 'selectms', text: '选择物资', icon: 'icon-glyph-shopping-cart', css: 'margin-left:8px;'}], onClick: onFormClick });
                if (_form.items['scanCode']) { var _scanCodeValue = $.Util.code.FullDate(); _form.items['scanCode'].setData(_scanCodeValue, _scanCodeValue); }
                if (_form.items['code']) { var _code = 'YHLL' + $.Util.code.FullDate(); _form.items['code'].setData(_code, _code); }
    ...
    function onWFComplete(obj) {
        MConfirm.setWidth(250).show('流程完成将会修改仓库库存信息！').evt('onOk', function () {
            $.Util.ajax({ args: 'm=SYS_CM_WH&action=onYHSendComplete&tkId=' + tkId });
        });
    }
    function onWFLoad() {
        var _eMS = taskInfo.addPanel({ title: '物资详情' }).css('position: relative;border-left:1px solid #e0e0e0;');
        new $.UI.List({ p: _eMS, ifFixedHeight: false, ifBindID: false, sonsKey: 'detail.sons', pidKey: 'detail.pid', style: 'tree:ms.scanCode', ifEnabledFilter: false, aHeader: _msDetailAry, loadApi: 'm=SYS_CM_WH&action=getMSDetailForTK&tkId=' + tkId + '&type=' + args.tid + '&jsonCondition={"detail.pid":0}', colControls: { header: {}} });
    }
