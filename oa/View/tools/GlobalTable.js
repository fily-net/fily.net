...
$View.tools.GlobalTable = function (args) {
    ...
    function _layout() {
        ...
        var _fiAry = [
            ...
            { name: 'attrObj', title: '属性对象', comType: 'Json' }
        ];
        var comArgs = {
            'root': { head_h: 38, title: '系统全局对照表管理', icon: 'icon-glyph-th-list', cn: 'b0' },
            'rootLayout': { min: 200, max: 500, isRoot: 1, start: 320, dir: 'we', dirLock: 2 },
            'infoForm': { title: '基本信息', icon: 'icon-vector-info-card', updateApi: 'm=SYS_TABLE_TREE&table=SYS_CM_GLOBAL_TABLE&action=updateNodeByID', loadApi: 'm=SYS_TABLE_TREE&table=SYS_CM_GLOBAL_TABLE&action=getNodeByID', ifFixedHeight: false, head_h: 30, items: _fiAry },
            'gtList': { url: 'View/common/TreeList.js', table: 'SYS_CM_GLOBAL_TABLE', rootID: args.rootID, onLoad: onGTLoad }
        }
        var struct = {
            p: owner,
            type: 'Tips',
