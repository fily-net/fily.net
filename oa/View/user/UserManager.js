$.namespace('$View.user');
$View.user.UserManager = function (args) {
    var me = this, coms, owner, _fn = function () { };
    var _args = { p: $DB };
    var formBtn, infoF, toolBar, uList;
    function _default() { }
    function _layout() {
        var _userHAry = [
            { name: 'cb', type: 'checkbox', width: 50 },
            { name: 'uid', type: 'none', title: '用户名', width: 120, ifFilter: true, ifDrag: true, filterItems: ['like'] },
            { name: 'cName', type: 'none', title: '中文名', width: 120, ifDrag: true },
            { name: 'eName', type: 'none', title: '英文名', width: 80, ifDrag: true },
        ...
        var _userInfo = [
            ...
            { name: 'uid', title: '用戶名', comType: 'Input', group: 'g1', req: true, sErr: '用户名必填' },
        ...
        var comArgs = {
            'userTips': { head_h: 37, title: '用户管理', icon: 'icon-glyph-user', cn: 'b0', onToolBarMouseDown: onToolBarMouseDown, onToolBarClick: onToolBarClick, gbsID: 19 },
            'userList': { aHeader: _userHAry, ifEnabledFilter: true, loadApi: 'm=SYS_TABLE_BASE&table=SYS_CM_USER&action=pagingForList&delFlag=0&jsonCondition={"id,!":1}', colControls: { header: {}, paging: {} }, onTDClick: onUserClick, onSuccess: function (obj) { if (obj.Length) { obj.List.fireClick(0); } else { toolBar.fireClick(0); } } },
            'userForm': { head_h: 37, icon_h: 18, loadApi: 'm=SYS_TABLE_BASE&action=getByID&table=SYS_CM_USER', insertApi: 'm=SYS_CM_USERS&action=addUser', updateApi: 'm=SYS_CM_USERS&action=updateUser', title: '员工基本信息', icon: 'icon-vector-info-card', items: _userInfo, onFinishInit: onFormInit, onSubmitSuccess: onFormSubmitSuccess },
    ...
    function onToolBarClick(obj) {
        switch (obj.Name) {
            case '20':
                ...
                formBtn.setIcon('icon-glyph-plus-sign').setText('添加用户信息');
                break;
            case '21':
    ...
    function delayShowInfo(id) {
        ...
        infoF.loadDataByID(id, function () { /* formBtn.setIcon('icon-glyph-edit').setText('修改用户信息');*/ });
    }
