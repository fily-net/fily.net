...
$View.workflow.FlowChart = function (args) {
    ...
    var toolAry = [
        { name: 'newChart', text: '创建', icon: 'icon-glyph-plus-sign' },
        { name: 'rePaint', text: '重画', icon: 'icon-glyph-repeat' },
        { name: 'toJSON', text: '输出成JSON', icon: 'icon-compact-tree-list' },
        { name: 'save', text: '保存', skin: 'btn-success' },
        { name: 'toDefinition', text: '生成定义', icon: 'icon-glyph-random', skin:'btn-warning' },
        { name: 'reToDefinition', text: '重新生成定义', icon: 'icon-glyph-repeat', skin: 'btn-warning' },
        { name: 'refresh', text: '刷新结构', icon: 'icon-glyph-refresh', skin: 'btn-info' }
    ];
    var loginT = { 0: '不显示, 只走选择用户与角色', 1: '显示选择用户与角色', 2: '显示所有用户与角色', 3: '不显示, 只转给发起人', 4: '走前一步所有Owner' };
    var logicItems = [
    ...
    function _layout() {
        ...
        var comArgs = {
            ...
            'infoForm': { head_h: 30, foot_h: 0, ifFixedHeight: false, title: '基本属性', icon: 'icon-glyph-list', items: fcAry, onChange: onInfoChange },
            'extForm': { head_h: 30, foot_h: 0, ifFixedHeight: false, title: '规则属性', icon: 'icon-glyph-align-justify', css: 'border-top:1px solid #EEEEEE;', onChange: onExtChange },
    ...
    function onFCLineClick(obj) {
        var _line = obj.Line;
        $.Dialog.get({ ifClose: true, css: 'max-width:165px;padding:10px;', ifFixedHeight: false, ePop: obj.eTxt }).init({
            type: 'Form', ifFixedHeight: false, state: 'Insert',
            btnItems: [{ name: 'FORM-SYS-SUBMIT', text: '编辑(Edit)', icon: 'icon-glyph-edit', align: 'right'}],
            items: [{ name: 'txt', comType: 'TextArea', ifHead: false, req: true}]
        }).evt('onSubmit', function (j) { var _dObj = j.Data.IText; _line.set('txt', _dObj['txt']); $.Dialog.destroy(); return false; }).show();
    }
