...
$View.project.PMInfo = function (args) {
    var me = this, coms, owner, _fn = function () { };
    var _args = { p: $DB, proId: 0, proList: null };
    var taskInfo, infoTips, toolBar, subBtn, popTips, infoBtns, fileList, fBtns, infoVal, currState, ifEdit = false, _pCost;
    var _fiAry = [
    ...
    function onInit() {
        subBtn.setText('修改工程信息').setIcon('icon-glyph-edit');
        ...
        taskInfo.loadDataByID(args.proId, function (obj) {
            ...
            if (_wfId) { infoBtns.addItem({ text: '查询分包商申请流程', icon: 'icon-glyph-search', name: _wfId, onClick: onSearchClick }); }
            if (_accountWfId) { infoBtns.addItem({ text: '查询分包费用结算流程', icon: 'icon-glyph-search', name: _accountWfId, onClick: onQueryAccount }); }
            switch (+_vObj.state) {
                case 506:  //未开工
                ...
                case 508:  //竣工
                    if (+obj.Value.ifRights) {
                        ...
                        fBtns.show();
                        fBtns.items[0].hide();
                        fBtns.addItem({ name: 'applyWF', text: '发起分包费用结算', css: 'margin-left:105px;', skin: 'Button-blue', onClick: applyWF });
                    } else {
                        fBtns.hide();
                    }
                    break;
                ...
                case 510:  //送审
                    ...
                    infoBtns.addItem({ text: '工程费用结算', icon: 'icon-glyph-search', name: 'account', onClick: onAccount });
                    break;
                case 511:   //已结
                    infoBtns.addItem({ text: '工程费用结算', icon: 'icon-glyph-search', name: 'account', onClick: onAccount });
                    infoBtns.show(); fBtns.hide();
                    infoBtns.items['116'].show();
                    infoBtns.items['117'].show();
                    infoBtns.items['118'].show();
                    infoBtns.items['119'].show();
    ...
    function onDesignClick(obj) {
        ...
        $.global.arrowTips.init({ type: 'View', url: 'View/project/ProjectDesignMete.js', proId: args.proId, cost: _pCost, dType: 0, onSubmitSuccess: function () { _pCost = 100; } });
    }
