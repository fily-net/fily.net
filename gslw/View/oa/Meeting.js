$.NameSpace('$View.oa');
$View.oa.Meeting = function (j) {
    var me = this, _fn = function () { };
    var owner;
    var args = { p: $DB, gtRootID: 145 }, eCal;
    var coms = {}, _now = new Date(), _year = _now.getFullYear(), _month = _now.getMonth() + 1, _day = _now.getDate(), _popTips, cal;
    var highCn = 'bc_20', selCn = 'bc_22', currSelCn = 'b_1', normalRCn = 'bc_33 b_23', selDate = getNow();
    var eDate, curr;
    function setDefault(j) { args = $.Util.initArgs(j, args); _month = _month < 10 ? '0' + _month : _month; _day = _day < 10 ? '0' + _day : _day; }
    function layout() {
        owner = args.p.adElm('', 'div').cn('wp hp');
        var comArgs = { 'root': { head_h: 32, foot_h: 0, cn: 'b0', title: '会议设置(<font color="red">提示：添加会议默认是添加当天会议</font>)', icon: 'fa fa-icon-list-ol', gbsID: 11, toolBarSkin: 'Button-default', onToolBarClick: onToolBarClick} }
        var struct = { p: owner, type: 'Tips', name: 'root' }
        coms = $.Util.initUI({ args: comArgs, struct: struct }); eCal = coms.root.body;
        loadByMonth();
    }
