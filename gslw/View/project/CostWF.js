...
$View.project.CostWF = function (j) {
    ...
    function onRights(wfInfo, toolBar, currNode, parentNode) {
        ...
        $.Util.ajax({
            args: 'm=SYS_CM_PRO_REPORT&action=getProjectCost&dataType=json&proId=' + args.info.id,
            onSuccess: function (obj) {
                ...
                var _obj1 = eval(obj.data[0] || '[]')[0],
                    _obj2 = eval(obj.data[1] || '[]')[0];
                console.log(obj.data);
                //addProjectMeta(_jiesuanparent, 0);
                addMSCost(_msparent, _obj1);
                addProjectMeta(_jiesuanparent, _order, _obj2);
                _msCost = (+_msCost).toFixed(2);
                _rgCost = (+_rgCost).toFixed(2);
                var _allCost = ((+_msCost) + (+_rgCost)).toFixed(2);
    ...
    function addMSCost(parent, cost) {
        _msCost = cost.jiaShuiHeJi || 0;
        var _title = '<div class="info-title">供应商费用(<font color="red">查看详情你进入领料详情</font>)</div><span class="info-tag">【无税金额：' + (cost.wuShuiJinE || 0) + '￥】</span><span class="info-tag">【价税合计：' + _msCost + '￥】</span>';
        var _taskAry = [
            { name: 'id', type: 'attr' },
        ...
        var list = new $.UI.List({
            ...
            aHeader: _taskAry,
            loadApi: 'm=SYS_TABLE_BASE&table=PROJECT_CAIGOU&action=pagingForList&jsonCondition={"proId": ' + args.proId + '}',
    ...
    function addProjectMeta(parent, order, cost) {
        _rgCost = cost.cost || 0;
        var _title = '<div class="info-title">工程量' + (order === 1 ? '【双击可以编辑单价】' : '') + '</div><span class="info-tag">【实际人工费用：' + (cost.cost || 0) + '￥】</span><span class="info-tag">【设计量费用：' + (cost.planCost || 0) + '￥】</span>';
        var _taskAry = [
            { name: 'id', type: 'attr' },
