$.NameSpace('$View.pm');
$View.pm.IndexNormal = function (j) {
    ...
    function layout() {
        ...
        var comArgs = {
            ...
            'toolTab': { gtID: 186, ifRights: true, itemAlign: 'right', items: [{ name: 'toUL', text: '切换到上下模式', icon: 'icon-glyph-resize-vertical'}], skin: 'ButtonSet-default mr10', onClick: onToolClick },
            'mainList': { aHeader: hAry, colControls: { header: {}, paging: { pageSize: 20, pageIndex: 1} }, onTDDoubleClick: onListDClick }
        }
        var struct = {
            p: owner,
            type: 'BaseDiv',
    ...
    function onTypeClick(obj) {
        ...
        mainList.loadAjax({ args: 'm=SYS_CM_PM&action=pagingForRightsPM' + _jc, cbFn: { onSuccess: function (obj) { if (obj.Length && args.fireid) { obj.List.fireClick(args.fireid, 'ID'); } } } });
    }
    function onListDClick(obj) { new $.UI.View({ p: args.p, title: obj.getAttr('nodeName'), type: 'normal', url: args.infoUrl, proId: obj.getAttr('rowid') }); }
    function onToolClick(obj) {
        if (obj.Name == 'toUL') { new $.UI.View({ p: args.p, url: 'View/pm/Index.js' }); return; }
        switch (+obj.Name) {
            case 189:
                //新建项目
                var infoAry = [
                ...
                popTips.Form = new $.UI.Form({ p: popTips.body, insertApi: 'm=SYS_TABLE_BASE&table=SYS_PM_PROJECT&action=addRow', items: infoAry, ifFixedHeight: false, onSubmitSuccess: function () { popTips.remove(); popTips = null; mainList.refresh(); onLoadTypeSuccess(); } });
                break;
            case 190:
                //删除项目
